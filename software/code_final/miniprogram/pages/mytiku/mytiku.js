...
 onLoad() {
  ...
    db.collection("my_questionList").doc(app.globalData.globalUserid)
    .get().then(res=>{
        console.log('res',res)
        app.globalData.globalPhase = res.data.phase
        app.globalData.globalSub = res.data.subject
  ...
  //       console.log("this.data.phase1",this.data.phase)
  ...
    })  
    ...
    if(app.globalData.globalSub==0){
      ...
      db.collection(this.data.kemu).doc(app.globalData.globalUserid).get().then(
          getnum => {
              this.setData({
