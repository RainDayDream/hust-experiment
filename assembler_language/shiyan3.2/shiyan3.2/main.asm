...
.model flat,c
	...
	input_compare proto c:dword,:dword
	compute proto :sdword,:sdword,:sdword,:dword
	mid_output proto c:sdword,:dword
	copy_num proto :dword,:dword
	assign_again proto c:dword,:dword,:dword
	SAMPLES  STRUCT
...
main proc
	input_lop:
		invoke input_compare,offset user_name,offset user_password
		cmp eax,0
		je exit
lopk:
	mov low_num,0
	...
	shuchu:
		...
		add ecx,13
		add edx,17
		invoke assign_again,ebx,ecx,edx
		CMP EAX,1
		je lopk
	exit:
